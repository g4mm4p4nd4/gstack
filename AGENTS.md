...
bun run upstream:sync    # start a safe merge from garrytan/gstack
```
...
- Keep `origin` pointed at `g4mm4p4nd4/gstack` and `upstream` pointed at `garrytan/gstack`; use `bun run upstream:sync` for future upstream refreshes.
