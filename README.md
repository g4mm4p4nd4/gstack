...
| `/pos-run-qa` | **Portfolio QA Lead** | QA a Portfolio OS dispatch or selection snapshot, using the scaffold or run target and writing run-scoped evidence files. |
| `/pos-evidence-backfill` | **Evidence Researcher** | Fill missing market or VOC evidence for a Portfolio OS run and write the inbox artifact back to portfolio-os. |
...
### Portfolio OS automation entrypoints

The local fork keeps Portfolio OS flywheel hooks as first-class gstack skills and scripts. These are intentionally local to `g4mm4p4nd4/gstack`; upstream refreshes should preserve them unless they are deliberately replaced by an equivalent upstream feature.
...
gstack is the reusable skill pack in the flywheel, not the scheduler. Portfolio OS owns recurring truth-loop automations, and Paperclip owns recurring run-health routines after dispatch ingest.

### Maintaining this fork

This checkout tracks two remotes: `origin` is `https://github.com/g4mm4p4nd4/gstack.git`, and `upstream` is `https://github.com/garrytan/gstack.git`. Pull upstream with a merge, not a rebase, so local commits remain visible and future conflict resolution has the right ancestry.

```bash
bun run upstream:sync
```

The sync script verifies remotes, refuses to run with dirty tracked files, fetches both remotes, creates a safety branch, and starts a no-commit merge from `upstream/main`. Resolve conflicts by taking upstream for generated/core gstack changes and reapplying local-only Portfolio OS entries where needed, then run:
